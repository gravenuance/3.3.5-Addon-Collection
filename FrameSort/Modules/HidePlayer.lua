...
local function ShowHide(show)
    ...
    for _, frame in ipairs(all) do
        ...
        local hasAttribute = frame.framesort_state_visibility == false

        if ( not show and wow.UnitIsUnit(unit, "player") ) then
            if ( not hasAttribute ) then
                -- user has opted to hide the player unit frame
                wow.UnregisterUnitWatch(frame)
                frame:Hide()
                frame.framesort_state_visibility = false
            end

            hasAttribute = nil
        end

        if ( hasAttribute and not frame:IsVisible() and wow.UnitExists(unit) ) then
            ...
            frame:Show()
            frame.framesort_state_visibility = nil
        end
    end
end
