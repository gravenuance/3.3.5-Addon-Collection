...
-- atlasTextures definition above is unchanged

local atlasTextures            = atlasTextures -- keep as is if already global/local

-- Cached locals for 3.3.5 performance
local unpack                   = unpack
local type                     = type
local error                    = error

-- Optional: exported table if you want programmatic access
Atlas                          = Atlas or {}
Atlas.textures                 = atlasTextures

local function NormalizeTexCoord(assetInfo, texCoordInfo)
    local left, right, top, bottom = unpack(texCoordInfo)

    local w = assetInfo.width
    local h = assetInfo.height
    ...
    return u1, u2, v1, v2, (right - left), (bottom - top)
end

function SetAtlasTexture(texture, textureName)
    if not texture or not textureName then
        return
    end

    local atlasInfo = atlasTextures[textureName]
    if not atlasInfo then
        -- silently skip unknown atlas names to avoid runtime errors
        return
    end
    ...
    if type(assetInfo) ~= "table" or type(texCoordInfo) ~= "table" then
        -- corrupt data; fail fast in dev, but crash‑safe if left as return
        -- error("Invalid atlas info for " .. tostring(textureName))
        return
    end

    local u1, u2, v1, v2, width, height = NormalizeTexCoord(assetInfo, texCoordInfo)
