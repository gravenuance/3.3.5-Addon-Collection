...
function C_Item.IsLocked(ItemLocation)
	...
	if ( EquipmentSlotIndex ) then
		Locked = IsInventoryItemLocked(EquipmentSlotIndex) and true or false
	else
...
function C_Item.GetItemInventoryType(ItemLocation)
	-- NOTE: Enum.__InventoryTypeInfo is keyed by Enum.InventoryType.Index* values,
	-- not by physical equipment slot IDs (they only coincidentally match for the
	-- first few slots) - route through the item's own EquipLoc instead, which
	-- GetItemInventoryTypeByID already resolves correctly.
	return C_Item.GetItemInventoryTypeByID(C_Item.GetItemID(ItemLocation))
end
...
function C_Item.DoesItemExist(ItemLocation)
	return C_Item.GetItemID(ItemLocation) ~= nil
end
