...
local DAY, HOUR, MINUTE = 86400, 3600,
	60               --used for formatting text
local DAYISH, HOURISH, MINUTEHALFISH, MINUTEISH, SOONISH = 3600 * 23.5, 60 * 59.5, 89.5, 59.5,
	5.5              --used for formatting text at transition points
...
local function ResyncCooldown(timer)
	local cd = timer.cooldown
	if not cd or not cd:IsShown() then
		timer:Stop()
		return
	end
	...
	if parent.action then
		start, duration, enable = GetActionCooldown(parent.action)

		-- Spell flyout / spell button example (adapt to your actual button types)
	elseif parent.spellID then
		start, duration, enable = GetSpellCooldown(parent.spellID)

		-- Item cooldowns (example)
	elseif parent.itemID then
		start, duration, enable = GetItemCooldown(parent.itemID)
	end

	if start and duration then
		if enable == 1 and start > 0 and duration >= OmniCC:GetMinDuration() then
...
do
	...
	f:SetScript('OnEvent', function()
		Timer:ForAllShown(function(timer)
			ResyncCooldown(timer)
		end)
	end)
